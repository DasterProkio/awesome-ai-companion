...
- [Miru](https://github.com/kiyotakali/Miru) - Self-hosted AI companion for macOS and Android with a Live2D desktop pet, screen-aware activity sensing, auditable Markdown memory, nightly memory organization, proactive messaging through its AttentionEngine, and multi-device sync through a user-run server. The repository currently ships apps, a server image, and documentation; full backend source is still being opened in stages. Apache-2.0. `macOS/Android` · `Self-host` · `ready`
