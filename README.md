...
- [dylan-heartbeat](https://github.com/callie0313/dylan-heartbeat) - Kelivo plugin that periodically wakes the companion, injects proactive behavior context, preserves timeline continuity, and sends Bark push messages when the AI chooses to reach out. `JavaScript` · `Kelivo` · `adapt`.
...
- [Listening Bridge](https://github.com/yoruuuchan/listening-bridge) - Self-hosted MCP bridge that exposes the active media session on Android or Windows to an AI companion, with fresh track context, synchronized-lyrics context, and explicit playback controls. The Android companion keeps an outbound WebSocket to a hosted service so it remains reachable away from the workstation. MIT. `Python/Java` · `Android/Windows/Self-host` · `ready`.
