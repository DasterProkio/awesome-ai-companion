...
def render(dates):
    ...
    for index, date in enumerate(dates, 1):
        ...
        y = top + chart_h - (index / max(count, 1)) * chart_h
        points.append((x, y))
    if not points:
        points = [(left, top + chart_h)]
    line = " ".join(f"{x:.1f},{y:.1f}" for x, y in points)
    area = f"{left:.1f},{top + chart_h:.1f} {line} {points[-1][0]:.1f},{top + chart_h:.1f}"
    year_labels = []
  ...
  <circle cx="{width - 72}" cy="58" r="24" fill="#f9c74f" opacity=".14"/>
  <text x="40" y="46" class="title">✦ {title}</text><text x="40" y="66" class="subtitle">{subtitle}</text>
  ...
  <polygon points="{area}" fill="url(#fill)"/><polyline points="{line}" fill="none" stroke="url(#line)" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" filter="url(#glow)"/>
  ...
  {''.join(year_labels)}
  <text x="40" y="{height - 30}" class="footer">Generated from GitHub stargazers · github.com/{OWNER}/{REPO}</text>
